...
double fillTime(tpool pool) {
    double flowRate = 0;
    int numInPipes = sizeof(pool.inPipes) / sizeof(tpipes),
        numOutPipes = sizeof(pool.outPipes) / sizeof(tpipes);
    for (int i = 0; i < numInPipes; i++)
        // Q = v · π · (d / 2)²
        flowRate +=
            pool.inPipes[i].flow * M_PI * pow(pool.inPipes[i].diameter / 2, 2);
    for (int i = 0; i < numOutPipes; i++)
        flowRate -= pool.outPipes[i].flow * M_PI *
                    pow(pool.outPipes[i].diameter / 2, 2);
